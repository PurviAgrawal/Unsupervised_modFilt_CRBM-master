function [model, output] = trainCRBM_Scale(data, params)
% % trainCRBM_Scale  Trains a convolutional restricted Boltzmann machine 
%   with the specified parameters.
%
%   [model output] = trainCRBM_Scale(data, params)
%
...
for iter = model.iter+1:param_iter,
    ...
    if ~rem(iter, param_iter),
            ...
            model.iter = iter;
            % For 1st Scale filter learning (otherwise comment)
            save('data/model_scale/model_clean_mel_scale_8tap_1', 'model', 'iter','error_iter');
            
            % For 2nd Scale filter learning (otherwise comment)
            % save('data/model_scale/model_clean_mel_scale_8tap_2', 'model', 'iter','error_iter');
            
            % For 3rd Scale filter learning (otherwise comment)
            % save('data/model_scale/model_clean_mel_scale_8tap_3', 'model', 'iter','error_iter');
